...
```
Copiar la plantilla vacía de zona DNS para crear la zona RPZ
```bash
sudo cp /etc/bind/db.empty /etc/bind/db.rpz.local
```
Editar el archivo de base de datos de la zona RPZ
```bash
...
@       IN      NS      localhost.
```
Editar el archivo principal de configuración de BIND9
```bash
...
// Incluir archivos de configuración de BIND9
include "/etc/bind/named.conf.options";
include "/etc/bind/named.conf.local";
include "/etc/bind/named.conf.default-zones";
```

Crear directorio donde se almacenarán los logs de BIND9
```bash
sudo mkdir /var/log/named/
```
Asignar permisos al usuario bind para escribir en el directorio
```bash
sudo chown bind:bind /var/log/named/ -R
```
Verificar la sintaxis general de configuración de BIND9
```bash
sudo named-checkconf
```
Verificar la sintaxis de la zona RPZ
```bash
sudo named-checkzone rpz /etc/bind/db.rpz.local
```
Reiniciar el servicio BIND9 para aplicar cambios
```bash
sudo systemctl restart bind9
```
Editar archivo de opciones de arranque de named
```bash
