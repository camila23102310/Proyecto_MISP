...
file_path = 'feeds.json'  # O proporciona la ruta completa si no está en el mismo directorio

enabled_feed_urls = []
feed_ids = []
with open(file_path, 'r') as file:
    data = json.load(file)  # Carga el contenido del archivo JSON
        ...
        if response.status_code == 200:
            if url.endswith('txt') or url.endswith('csv'):
                ...
                with open(url_file, 'a') as url_out: 
                    if urls:
                        url_out.write(','.join(urls) + ',')
                if os.path.exists(archivo_salida):
                    os.remove(archivo_salida)

            else: print("El archivo no es de texto o CSV.") 
        else: 
            print("Error al acceder a la URL.")
    except Exception as e: print("Ocurrió un error:", e)
# Imprime los IDs de los feeds habilitados
print("\nIDs de los feeds habilitados:")
